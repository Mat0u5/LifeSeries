...
public class SuperpowersWildcard extends Wildcard {
    ...
    public static void rollRandomSuperpowers(List<ServerPlayer> allPlayers) {
        ...
        if (!WILDCARD_SUPERPOWERS_DISABLE_INTRO_THEME) {
            PlayerUtils.playSoundToPlayers(allPlayers, SoundEvent.createVariableRangeEvent(IdentifierHelper.lifeseries("wildlife_superpowers")), 0.2f, 1);
        }
    }
    ...
    public static void setSuperpower(ServerPlayer player, Superpowers superpower) {
        ...
        if (!WILDCARD_SUPERPOWERS_DISABLE_INTRO_THEME) {
            PlayerUtils.playSoundToPlayer(player, SoundEvent.createVariableRangeEvent(IdentifierHelper.lifeseries("wildlife_superpowers")), 0.2f, 1);
        }
        Necromancy.checkRessurectedPlayersReset();
    }
    ...
    public static Superpower.KeyPressResult pressedSuperpowerKey(ServerPlayer player) {
        if (playerSuperpowers.containsKey(player.getUUID())) {
            if (((IPlayer) player).ls$isAlive()) {
                ...
                Superpower.KeyPressResult result = power.onKeyPressed();
                if (result == Superpower.KeyPressResult.STOLEN) {
                    Mimicry.displayStolenPowerActionbar(power, player);
                }
                return result;
            }
            else {
