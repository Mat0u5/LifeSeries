...
public class ModifiableText {
    ...
    public static void initialize() {
        ...
        if (currentSeason.getSeason() == Seasons.THIRD_LIFE) {
            register("boogeyman.test", "#7Blah Blah boogeyman in THIRD LIFE WWWATTT");

        }
        else {
            register("boogeyman.test", "#7Blah Blah #cboogeyman #7:)");
        }

        register("args.rawr", "#a%1$s#a send a message (%3$s) to %4$s", List.of("Player", "Receiver", "Message"));
        register("args.rawr2", "#a%s send a message", List.of("Player"));
    }
